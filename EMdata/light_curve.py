...
##this need a real path because we will check if some file exist before

path_dir =  f"{os.path.dirname(os.path.realpath('__file__'))}/Farah_lc"
    ...
    for dist in distribution :
        for telescope in tqdm(telescopes):
            for run_name in run_names:
                for pop in pops:
                    job, s = 0, 0
                    path = Path(f'{path_dir}/outdir_{pop}/{pop}')
