...
data_dir ='/home/weizmann.kiendrebeogo/OBSERVING_SCENARIOS/observing-scenarios-2022/Farah'
datapath='/home/weizmann.kiendrebeogo/OBSERVING_SCENARIOS/observing-scenarios-2022/Results/Figures/farah'
...
with tqdm(total=len(run_dirs) * len(pops)) as progress:
    for run_name, run_dir in zip(run_names, run_dirs):
        for pop in pops:
            path =  Path(f'{data_dir}/runs') / run_dir / ('farah_' + pop.lower())
            #if pop.lower() == 'farah':
             #   path = Path('runs') / run_dir / (pop.lower())
            #else:
             #   path = Path('runs') / run_dir / (pop.lower() + '_astro')
...
## Ligo.skymap inintial sampling

Leo_tables ={}
for pop in tqdm(['BNS', 'NSBH', 'BBH']):
    leo_path = f'/home/weizmann.kiendrebeogo/OBSERVING_SCENARIOS/observing-scenarios-2022/Leo/runs/O3/{pop.lower()}_astro/'
    allsky = Table.read(f'{leo_path}/allsky.dat', format='ascii')
    injections = Table.read(f'{leo_path}/injections.dat', format='ascii')
    coincs = Table.read(f'{leo_path}/coincs.dat', format='ascii')
    table = join(allsky, coincs, 'coinc_event_id')
    table = join(table, injections, 'simulation_id')
    table['vol(90)'] *= 1e-6
    Leo_tables[pop] = table
    del allsky, injections, coincs, table
...
for irun, (run_name, tables1) in enumerate(tables.items()):
    for ipop, (pop, table) in enumerate(tables1.items()):
        for ifield, fieldname in enumerate(fieldnames):
            ...
            #Remove zero in data to ovoid to get inf in kde ... 
            indice_nonzero= np.nonzero(data)
            data=data[indice_nonzero]
            ax = axs[ifield]
...
for ax, fieldname in zip(axs, fieldnames):
    ax.figure.savefig(f'{datapath}/{fieldname}.pdf')
...
for pop, color, axrow in zip(['BNS', 'NSBH', 'BBH'], classification_colors, axs):
    for fieldname, ax in zip(fieldnames, axrow):
        ...
        medians = []
        for data, label, linewidth, linestyle, color_weiz in [
                    [old_tables[pop][fieldname], 'LRR', 0.5 * plt.rcParams['lines.linewidth'],'--', 'b'],
                    [Leo_tables[pop][fieldname], 'bayestar-inject',  plt.rcParams['lines.linewidth'], ':', 'g'],
                    [tables[run_name][pop][fieldname], 'R&P', 0.5*plt.rcParams['lines.linewidth'],'-', 'r']]:
            data = data[np.isfinite(data)]
            #Remove zero in data to ovoid to get inf in kde ... 
            indice_nonzero= np.nonzero(data)
            data=data[indice_nonzero]
            ...
            t = np.geomspace(*ax.get_xlim(), 100)
            y = stats.norm(kde.dataset.ravel(), std).cdf(np.log(t)[:, np.newaxis]).mean(1)
            ax.plot(t, y, color=color_weiz, linewidth=linewidth, label=label, linestyle=linestyle)

        #if fieldname != 'distance':
          #  ax.annotate( '', (medians[2], 0.5), (medians[0], 0.5),
           #     arrowprops=dict(arrowstyle='<|-', color=color, shrinkA=4, shrinkB=4,
...
fig.align_labels()
fig.savefig(f'{datapath}/o3-comparison.pdf')
...
fig, axs = plt.subplots(
    len(pops), len(fieldnames),sharex='col', sharey=True, gridspec_kw=dict(bottom=0.08, left=0.08, top=0.92, right=0.95),figsize=(7.3, 6))
...
axs[-1, -1].legend()
fig.align_labels()
fig.savefig(f'{datapath}/predictions.pdf')
...
for pop, color, ax in zip(pops, classification_colors, axs[:, 0]):
    ax.text(
        0.05, 0.95, pop,
        transform=ax.transAxes, color=color, va='top')
...
fig.align_labels()
fig.savefig(f'{datapath}/annual-predictions.pdf')
...
f = io.StringIO()
with open(f'{datapath}/summary.rst', 'w') as f_rst:
    print('+-----------+-----------+---------------+---------------+---------------+', file=f_rst)
    print('|           |           | Source class                                  |', file=f_rst)
    print('| Observing |           +---------------+---------------+---------------+', file=f_rst)
    print('| run       | Network   | Farah_BNS     | Farah_NSBH    | Farah_BBH     |', file=f_rst)
    print('+===========+===========+===============+===============+===============+', file=f_rst)
...
statfunc = np.nanmedian
f = io.StringIO()
with open(f'{datapath}/extremes.rst', 'w') as f_rst:
    print('+-----------+-----------+---------------+---------------+---------------+', file=f_rst)
    print('|           |           | Source class                                  |', file=f_rst)
    print('| Observing |           +---------------+---------------+---------------+', file=f_rst)
    print('| run       | Network   | Farah_BNS     | Farah_NSBH    | Farah_BBH     |', file=f_rst)
